...
CKEDITOR.dialog.add( 'customanchor', function( editor ) {
	...
	return {
		...
		onShow: function() {
			...
			if ( fullySelected ) {
				if ( CKEDITOR.plugins.customlink.fakeAnchor ) {
					var realElement = CKEDITOR.plugins.customlink.tryRestoreFakeAnchor( editor, fullySelected );
				...
				partialSelected = CKEDITOR.plugins.customlink.getSelectedLink( editor );
				if ( partialSelected ) {
		...
		contents: [
			...
			elements: [
				...
				validate: function() {
					if ( !this.getValue() ) {
						alert( editor.lang.customlink.anchor.errorName );
						return false;
					}
					return true;
				}
