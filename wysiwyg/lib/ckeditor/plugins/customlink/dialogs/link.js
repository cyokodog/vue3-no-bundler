...
CKEDITOR.dialog.add( 'customlink', function( editor ) {
	...
	var linkTypeChanged = function() {
			var dialog = this.getDialog(),
				partIds = [ 'urlOptions', 'anchorOptions', 'emailOptions', 'sitemapOptions' ],
	...
	var parseLink = function( editor, element ) {
			...
			if ( !retval.type ) {
				...
				// choose from sitemap
				else if ( href && ( sitemapMatch = href.match( sitemapRegex ) ) ) {
					retval.type = 'sitemap';
					retval.sitemap = {};
					retval.sitemap.page_id = sitemapMatch[ 1 ];
				}
			...
			if ( CKEDITOR.plugins.customlink.emptyAnchorFix ) {
			...
			if ( CKEDITOR.plugins.customlink.fakeAnchor ) {
				var imgs = editor.document.getElementsByTag( 'img' );
				for ( i = 0, count = imgs.count(); i < count; i++ ) {
					if ( ( item = CKEDITOR.plugins.customlink.tryRestoreFakeAnchor( editor, imgs.getItem( i ) ) ) )
						anchors.push({ name: item.getAttribute( 'name' ), id: item.getAttribute( 'id' ) } );
				}
			}

			// CMS用管理画面で追加したアンカーを収集
			var cmsAnchors = editor.document.getElementsByTag( 'a' );
			for ( i = 0, count = cmsAnchors.count(); i < count; i++ ) {
				var item = cmsAnchors.getItem( i );
				if ( item.getAttribute('class') === 'cms-admin-anchorFlag' ) {
					anchors.push({ name: item.getAttribute( 'id' ), id: item.getAttribute( 'id' ) } );
				}
			}
						...
						, children : [
							{
								id : "page"
								, type : "select"
								, label : linkLang.sitemap
								, "default" : ""
								, items : []
								, setup : function( data ){
									this.clear();
									var tree = cms.view.pageList.getFlattenTree();
									var collection = cms.collection.pages;
									for(var i = 0, n = tree.length; i < n; i++){
										var page = tree[i];
										var model = collection.get(page.id)
										if (model && model.get('data.deleted') != 'true') {
											this.add(model.get("name"), model.get("id"));
										}
									}
									if (data.sitemap) {
										this.setValue(data.sitemap.page_id || "");
									}
								}
								, commit : function( data ){
									if(!data.sitemap){
									...
									data.sitemap.page_id = this.getValue();
								}
							}
						]
					}
		...
		onOk: function() {
			...
			switch ( data.type || 'url' ) {
				case 'url':
					...
					break;
				case 'sitemap':
					var path = ( data.sitemap && data.sitemap.page_id != undefined ) ? '[page_id:' + data.sitemap.page_id + ']' : '';
					attributes[ 'data-cke-saved-href' ] = path;
					break;
				case 'anchor':
			...
			if ( !this._.selectedElement ) {
				...
				if ( range.collapsed ) {
					...
					var _text = data.type == 'email' ? data.email.address : attributes[ 'data-cke-saved-href' ];
					if (data.type == 'sitemap') {
						var _id = data.sitemap.page_id;
						var collection = cms.collection.pages;
						for(var i = 0, n = collection.length; i < n; i++){
							var model = collection.at(i)
							if (model.get('id') == _id && model.get('data.deleted') != 'true') {
								_text = model.get("name");
							}
						}
					}
					var text = new CKEDITOR.dom.text( _text, editor.document );
				...
				if ( data.adv && data.adv.advName && CKEDITOR.plugins.customlink.synAnchorSelector )
